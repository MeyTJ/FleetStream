...
| frontend | 3000 | Next.js dashboard (SSR + static assets) |
...
## Frontend image: build args, not runtime env

`frontend/` is a Next.js app: `NEXT_PUBLIC_*` values and the CSP `connect-src`
are **inlined at build time**, so the browser can only reach the BFF that the
image was built against. Building a staging/prod image therefore requires:
...
docker build \
  ...
  -t fleetstream/frontend:v1.0.0 .
```

Setting those variables on the Deployment instead has no effect. The
`Content-Security-Policy` header is generated by the same values (see
`frontend/src/lib/csp.ts`), so an image built without them keeps localhost in
`connect-src` and the dashboard will fail to reach the BFF.
